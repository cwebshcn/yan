...
function get_user_content($user_id){
	global $lnk;
	$result=$lnk -> query("select * from spread_user where openid<>'' and user_id='".$user_id."'"); 
...
function get_top3($objname){
	...
	echo ("select * from spread_user where item_id = $item_id and openid<>'' and parent_id>0  order by count_$objname desc limit 0,3");
	$result = $lnk -> query("select * from spread_user where item_id = $item_id and openid<>'' and parent_id>0  order by count_$objname desc limit 0,3");
...
function get_count($type="card",$user_uid,$item_id){
	...
	//数据库查询
	$sql ="select user_id,phone,avatar,nick_name,count_card,user_ip,user_ip_city,get_rtmp,get_rtc,get_spot,get_deal,get_deal_online from spread_user where openid<>'' $citysql and parent_id='$user_uid'";
	...
	*/
	...
	$lnk -> query("update  spread_user set count_$type = '$count_userall' where openid<>'' and user_id='".$userGroup["user_id"]."'");
