...
<div style="width:1024px;margin:0 auto;text-align: center;padding:25px;">
	<h3>用户手机号一览表（总计：<?php echo count_all_phone($pid);?>）</h3>
	<div class="row" >
		<div>&nbsp;</div>
	<?php 
		$result=$lnk -> query("select phone from spread_user where item_id='".$pid."' and phone>0 and openid<>'' order by rand()");
...
<?php 
	function count_all_phone($item_id){
		global $lnk;
		$result=$lnk -> query("select count(0) from spread_user where item_id='".$item_id."' and phone>0 and openid<>'' order by redeem_count_bc desc"); 
