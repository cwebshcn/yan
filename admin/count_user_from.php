...
<div style="width:1200px;margin:0 auto;text-align: center;padding:25px;">
	...
	<div id="data_list">
		<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3" style="font-size:14px;color:#333;border-bottom:1px solid #e0e0e0;line-height:18px;" v-for="l in list">
			<span  v-if="l.avatar"  style="display:inline-block" ><img :src="l.avatar" style="width:60px;height:60px;margin:3px;"/></span>
			...
			<span  style="display:inline-block;width:180px;text-overflow:ellipsis;white-space: nowrap;" > {{l.nick_name ? l.nick_name : ''}}   <br>{{l.phone>0? l.phone : ''}} </span></div>
	</div>
			...
			<?php 
				...
				//所有从分享页进的用户
			    $count_share_page = 0;
			    $first_user_id =get_first_user($pid);
			    

			    //所有从分享页进的用户组
			    $arr_share_page=array();
				$result=$lnk -> query("select count_card,count_deal,count_take,get_rtmp,user_id,phone,avatar,nick_name from spread_user where item_id='".$pid."' and parent_id>$first_user_id and enter_path='0' and openid<>'' order by phone desc");
				...
				$result=$lnk -> query("select count_card,count_deal,count_take,get_rtmp,user_id,phone,avatar,nick_name from spread_user where item_id='".$pid."' and parent_id>$first_user_id and enter_path='1' and openid<>'' order by phone desc");
				while ($rs=mysqli_fetch_assoc($result)){ 
					$count_liveroom_page++;
				...
				$result=$lnk -> query("select count_card,count_deal,count_take,get_rtmp,user_id,phone,avatar,nick_name from spread_user where item_id='".$pid."' and parent_id>$first_user_id and touch_phone='1' and openid<>'' order by phone desc");
				while ($rs=mysqli_fetch_assoc($result)){ 
					$count_click_phone++;
					$arr_click_phone[] = $rs;
				}
				...
				$result=$lnk -> query("select count_card,count_deal,count_take,get_rtmp,user_id,phone,avatar,nick_name from spread_user where item_id='".$pid."' and parent_id>$first_user_id and touch_wx_qrcode='1' and openid<>'' order by phone desc");
				while ($rs=mysqli_fetch_assoc($result)){ 
					$count_touch_qrocde++;
					$arr_touch_qrocde[] = $rs;
				}
...
<?php 
	...
	function get_first_user($item_id){
		global $lnk;
		$result = $lnk -> query("select user_id from spread_user where item_id='".$item_id."' and parent_id='0'");
		while ($rs=mysqli_fetch_assoc($result)){
			return $rs["user_id"]+0;
		}
		return 0;
