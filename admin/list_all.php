...
if($result){
	while ($rs=mysqli_fetch_assoc($result)){
		...
		$citylist.=($menuid==21)?"&nbsp;&nbsp;&nbsp;<a href='datelist.php?menuid=$menuid&cid=".$rs["id"]."'><span class='glyphicon glyphicon-calendar'></span></a>":"";
		echo "<td><a href='?act=addnew&menuid=$menuid'><span class='glyphicon glyphicon-plus'></span></a>&nbsp;&nbsp;&nbsp;<a href='?act=edita&sortid=$sortid&menuid=$menuid&pid=".$rs["id"]."'><span class='glyphicon glyphicon-pencil'></span></a>&nbsp;&nbsp;&nbsp;<a href='javascript:void(0)' onClick='confirmLink(".$rs["id"].")'><span class='glyphicon glyphicon-remove'></span></a>$citylist</td>";  //操作
	echo "</tr>";
	}
}
    ...
    <input type='button' name='Submit11' value='上传' onClick=\"window.open('./upload.php?formname=myform&editname=".$rs["name"]."&uppath=".$rs["name"]."&filelx=jpg','','status=no,scrollbars=no,top=20,left=110,width=400,height=100')\">";
				...
				case 6:
				 ...
				 $typea="<select name='".$rs["name"]."'>";				
					...
					break;
				case 7:
					$typea="<input name='".$rs["name"]."[]' type='text'  value='' size='10'><input name='".$rs["name"]."[]' type='text'  value='' size='10'><input name='".$rs["name"]."[]' type='text'  value='' size='10'><input name='".$rs["name"]."[]' type='text'  value='' size='10'><input name='".$rs["name"]."[]' type='text'  value='' size='10'>";
					break;
				case 8:
					$typea="<input name='".$rs["name"]."' id='".$rs["name"]."' type='text'  value='' size='40' readonly><script>$('#".$rs["name"]."').datepicker({
    format: 'yyyy-mm-dd',
    autoclose: true,
    minView: 'month',
    maxView: 'decade',
