...
<div style="width:1024px;margin:0 auto;text-align: center;padding:25px;">
	...
	<div id="data_list">
		<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3" style="font-size:14px;color:#333;border-bottom:1px solid #e0e0e0;line-height:32px;" v-for="l in list">{{l.nick_name}}  {{l.phone}}</div>
			...
			//所有用户
				$result=$lnk -> query("select count_card,count_deal,count_take,get_rtmp,user_id,phone,nick_name from spread_user where item_id='".$pid."' and phone>0 and parent_id>$first_user_id and openid<>'' order by redeem_count_bc desc");
				while ($rs=mysqli_fetch_assoc($result)){
				$count_all++;
				$arr_all[] =$rs;
				}
...
<?php 
	...
	function count_all_phone($item_id){
		global $lnk;
		$first_user_id =get_first_user($pid);
		$result=$lnk -> query("select count(0) from spread_user where item_id='".$item_id."' and phone>0 and parent_id>$first_user_id and openid<>'' order by redeem_count_bc desc"); 
