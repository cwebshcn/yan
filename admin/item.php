...
<?php if($action=="view" and $pid>0){?>
	...
	<td width="10%"><strong>身份</strong></td>
	...
	$result = $lnk -> query("select * from spread_user where item_id = $pid and openid<>''  order by parent_id=0 desc,px desc, count_userb desc,count_userc desc,count_card desc limit $start,$pagenum");
	while ($rs=mysqli_fetch_assoc($result)){
		...
		$kf=get_kf_phone($pid,$rs["phone"]);
		if($first_user_id==$rs["user_id"]){
			$sf="首发二维码";
		}elseif($rs["redeem_code"]=="witness"){
			$sf="见证用户组";
		}elseif($kf>0){
			$sf="客服";
		}elseif($first_user_id==$rs["parent_id"]){
			$sf="兼职";
		}else{
			$sf="普通用户";
		}
...
<?php if($action=="search" and $pid>0){
	...
	<tr><td align="center">
		...
		<?php }elseif($arr[0]["redeem_code"]=="witness"){

			$weburl = str_replace("admin/","",strtolower($weburl));
			$witness_qrcode=qrcode($weburl."witness.php?u=".$arr[0]["user_id"]);
			//echo $zz_qrcode;
			//$upload_qrcode=qrcode($weburl."wxqrcode.php?u=".$arr[0]["user_id"]);
			?>
			<div style="padding-bottom:35px;"><img src="temp/<?php echo $witness_qrcode;?>" width="200" height="200" id="f_code"><h3>见证二维码</h3></div>
...
//添加种子用户
$lnk -> query("insert into spread_user (parent_id,item_id,nick_name,phone,open_time,openid,open_status,redeem_code,px) values ('$user_id','$item_id','见证用户组','0','$reg_time','witness','1','witness','99999')"); 

$witness_id=mysqli_insert_id($lnk);
...
alert("您的数据已加入成功！");
go("?menuid=$menuid&witness=$witness_id");
}
}
?>
<?php
...
function get_kf_phone($item_id,$phone){
		global $lnk;
		$result = $lnk -> query("select * from spread_item_kefu where item_id='".$item_id."' and phone='".$phone."'");
		while ($rs=mysqli_fetch_assoc($result)){
			return $rs["id"]+0;
		}
		return 0;
